...
def interpret_game(game: int) -> None:
    image = cv.imread(f"./train/{game}_00.jpg")
    ...
    moves = [f"./train/{game}_{i:02d}.jpg" for i in range(1, MOVES_PER_GAME + 1)]
    ...
    for move_idx, move_path in enumerate(moves):
        ...
        board = get_board(image)
        board_padded = get_board(image, padding=50)
        ...
        # new_tiles = get_different_tiles(patches_curr, patches_next)
        ...
        for (i, j) in new_tiles:
            ...
            print(f"{row}{col} {shape}{color}")
        
        print(scorer.get_points(board_tiles, new_tiles))
        print()
